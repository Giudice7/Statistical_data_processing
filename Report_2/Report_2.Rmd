...
```
As you can easily see, just the first 9 components are responsible for almost the 90% of the variance in the dataset, as represented in the figure below:
...
```
In this case, only the first `r relevant_pc` principal components have been selected, which account for the 75% of the cumulative variance in the data.
...
Cluster results can be interpreted studying the variable distribution in the cluster. For example, in Cluster 5 there are buildings with lower performance (*classe energetica* equal to D, E, F) while Cluster 8 is composed mostly by buildings with high performance (*classe energetica* equal to A1, A2, A3, A4). Very often, the numerical variables for which the clustering has been performed, assume very different values. For instance, buildings of Cluster 8 have a lower ratio $A_{sol}/A_{sup}$, meaning that probably the summer conditioning required is lower; or the dynamic transmittance $Y_{ie}$ has lower value in Cluster 8 in comparison with Cluster 5, which means that the heat flux crossing the envelope is more mitigated in high performance buildings, decreasing the energy requirement. Furthermore, also the year of construction is important; in fact, buildings built recently have usually an higher performance.
